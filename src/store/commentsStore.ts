...
export const useCommentsStore = create<any>((set) => ({
    ...
    getComments: async (id) => {
        ...
        const json = await result.json()         //новость выбранная
            json?.kids?.forEach(async (idComment) => {  //каждый комментарий этой новости
                const resultComment = await fetch(`https://hacker-news.firebaseio.com/v0/item/${idComment}.json`)
